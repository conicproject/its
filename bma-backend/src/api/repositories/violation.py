...
from api.models.violation import ViolationModel
...
class ViolationRepository:
    ...
    def get_by_time_payloads(self, payload):
        data = ViolationModel.objects.filter(
            date=payload['date'],
    ...
    def get_total(self, payload):
        sql_command = '''
            SELECT SUM(VOLUME) AS total FROM BMA_PHASE_II.VIOLATION_RECORDS
    ...
    def get_sum_group_by_car_type(self, payload, checkpoint_id):
        sql_command = '''
            SELECT CAR_TYPE_ID, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS 
            WHERE CREATED_DATE = TO_DATE('{}', 'YYYY-MM-DD')
                AND TIME_RANGE_ID > {}
                AND TIME_RANGE_ID <= {}
                AND CHECKPOINT_ID = {}
    ...
    def get_sum_group_by_dir(self, payload):
        sql_command = '''
            SELECT DIRECTION, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS
            WHERE CREATED_DATE = TO_DATE('{}', 'YYYY-MM-DD')
                AND TIME_RANGE_ID > {}
                AND TIME_RANGE_ID <= {}
            GROUP BY DIRECTION
    ...
    def get_time_type_direction_by_date_range(self, start_date, end_date, checkpoint_id):
        ...
        # return data
        sql_command = '''
            SELECT TIME_RANGE_ID, DIRECTION, CAR_TYPE_ID, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS
            WHERE CREATED_DATE >= TO_DATE('{}', 'YYYY-MM-DD')
    ...
    def get_by_date_range(self, start_date, end_date, checkpoint_id):
        sql_command = '''
            SELECT DIRECTION, CREATED_DATE, CAR_TYPE_ID, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS
            WHERE CREATED_DATE >= TO_DATE('{}', 'YYYY-MM-DD')
                AND CREATED_DATE < TO_DATE('{}', 'YYYY-MM-DD')
                AND CHECKPOINT_ID = {}
    ...
    def get_volume_by_date_range(self, start_date,end_date, checkpoint_id):
        sql_command = '''
            SELECT CREATED_DATE , CAR_TYPE_ID, DIRECTION, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.RECORDS 
            WHERE CREATED_DATE >= TO_DATE('{}', 'YYYY-MM-DD')
                AND CREATED_DATE < TO_DATE('{}', 'YYYY-MM-DD')
                AND CHECKPOINT_ID = {}
            GROUP BY CAR_TYPE_ID,DIRECTION,CREATED_DATE
            ORDER BY CAR_TYPE_ID,DIRECTION,CREATED_DATE
        '''.format(start_date,
            end_date, 
            checkpoint_id
        )
    ...
    def get_by_year(self, year, checkpoint_id):
        ...
        # )
        sql_command = '''
            SELECT DIRECTION, EXTRACT( MONTH FROM CREATED_DATE ) AS CREATED_DATE, CAR_TYPE_ID, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS 
            WHERE EXTRACT(YEAR FROM CREATED_DATE) = '{}'
                AND CHECKPOINT_ID = {}
            GROUP BY DIRECTION, CREATED_DATE, CAR_TYPE_ID 
            ORDER BY DIRECTION, CREATED_DATE, CAR_TYPE_ID
    ...
    def get_volume_by_year(self, year, checkpoint_id):
        sql_command = '''
            SELECT EXTRACT( MONTH FROM CREATED_DATE ) AS CREATED_DATE,CAR_TYPE_ID,DIRECTION,SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.RECORDS
            WHERE EXTRACT(YEAR FROM CREATED_DATE) = '{}'
                AND CHECKPOINT_ID = {}
            GROUP BY CAR_TYPE_ID,DIRECTION,CREATED_DATE
            ORDER BY CAR_TYPE_ID,DIRECTION,CREATED_DATE
    ...
    # def get_volume_by_year(self, year, checkpoint_id):
    #     sql_command = '''
    #         SELECT SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.RECORDS 
    #         WHERE EXTRACT(YEAR FROM CREATED_DATE) = '{}'
    #             AND CHECKPOINT_ID = {}
    #     '''.format(
    #         year, 
    ...
    def get_inbound_checkpoint_time_range(self, payload, checkpoint_id):
        sql_command = '''
            SELECT TIME_RANGE_ID, CREATED_DATE, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS
            WHERE CREATED_DATE = TO_DATE('{}', 'YYYY-MM-DD')
                AND TIME_RANGE_ID > {}
                AND TIME_RANGE_ID <= {}
                AND CHECKPOINT_ID = {}
    ...
    def get_inbound_dashboard(self, payload):
        sql_command = '''
            SELECT DIRECTION, CHECKPOINT_ID, SUM(VOLUME) AS VOLUME FROM BMA_PHASE_II.VIOLATION_RECORDS
            WHERE CREATED_DATE = TO_DATE('{}', 'YYYY-MM-DD')
                AND TIME_RANGE_ID > {}
                AND TIME_RANGE_ID <= {}
                AND DIRECTION = 'IN'
