...
def camerafeed(request,passing_id):
    ...
    # violation_passing_id = request.GET.get('id', False)
    violation_passing_id = str(passing_id)
    if violation_passing_id:
        # data = OracleVehiclePassRepository.get_violation_report_by_id(
        #     id=violation_passing_id
        # )
        # pass_id = data.pass_id
        vehicle_data = OracleVehiclePassRepository().get_playback_data_by_violation_pass_id(
            pass_id=violation_passing_id
        )
        ...
        # start_time = pass_time - datetime.timedelta(seconds=34)
        # end_time = pass_time + datetime.timedelta(seconds=19)
