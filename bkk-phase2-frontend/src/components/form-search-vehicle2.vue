      ...
      <v-col>
        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-text-field
              hint="ช่วงเวลาที่ค้นหาข้อมูลสูงสุด 5 วัน"
              persistent-hint
              placeholder="วัน เดือน ปี พ.ศ."
              outlined
              class="input-adddata rounded-lg sl mb-2"
              v-model="dayformat"
              readonly
              v-bind="attrs"
              v-on="on"
    ...
    <v-row>
      <v-col
        ><v-text-field
          v-model="textLicense"
          outlined
          label="ป้ายทะเบียนยานพาหนะ"
          class="input-adddata rounded-lg sl"
          hint="กรณีไม่ทราบเลขทะเบียนแน่ชัด ให้ละค่านั้นด้วย x เช่น 2กx 8xx "
          persistent-hint
        ></v-text-field
      ></v-col>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    dateformat() {
      ...
      let dateformat = new Date(this.date).toLocaleDateString("th-TH", {
        year: "numeric",
        month: "numeric",
        day: "numeric",
      });
  ...
  methods: {
    ...
    confirm() {
      // let selectPointName = selectPoint.find(listCheckPoint => listCheckPoint.id == id).name;

      if (this.selectPoint) {
        this.selectPointName = this.listCheckPoint.find(
          (x) => x.id == this.selectPoint
        ).name;
      } else {
      ...
      // let selectPointName ="";
      // if(this.listCheckPoint.find(x => x.id == this.selectPoint)){
      // let selectPointName = this.listCheckPoint.find(x => x.id == this.selectPoint).name || "s";
      //   console.log("selectPointName",selectPointName)
      // }else{
      //   selectPointName = "222";
      //   console.log("selectPointName",selectPointName)
      // }
