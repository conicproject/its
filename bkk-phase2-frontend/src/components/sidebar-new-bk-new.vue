        ...
        <ul v-for="item in dataMenuBar" :key="item.name">
          <div :class="{ activeHeader: item.path === activeIdOther }">
            <div class="menu pointer" v-if="item.child_obj && item.child_obj.length > 0"
              ...
              <!-- Dynamic child menu display - เปลี่ยนจาก v-show เป็น v-if เพื่อแสดงเมนูย่อยที่เปิดอยู่ -->
              <div class="menuChild pointer" v-if="expandedMenus.includes(item.path)">
...
export default {
  ...
  data() {
    return {
      ...
      activeIdMenuChild: '',
      activeExpandId: null,
      expandedMenus: [], // เพิ่ม array สำหรับเก็บเมนูที่ถูกขยาย
    };
  },
  watch: {
    sideNav(newVal) {
  ...
  mounted() {
    ...
    this.loadExpandedMenus(); // โหลดเมนูที่ถูกขยายจาก localStorage
  },
  methods: {
    initializeActiveMenu() {
      ...
      if (pathToCheck) {
        ...
        if (parentMenu && (savedHeaderPath || currentPath)) {
          ...
          this.activeIdOther = '';
          // เพิ่มเมนูหลักที่มีเมนูย่อยที่ active อยู่ลงใน expandedMenus
          if (!this.expandedMenus.includes(parentMenu.path)) {
            this.expandedMenus.push(parentMenu.path);
          }
    ...
    loadExpandedMenus() {
      // โหลดรายการเมนูที่ถูกขยายจาก localStorage
    ...
    saveExpandedMenus() {
      // บันทึกรายการเมนูที่ถูกขยายลง localStorage
    ...
    handleMenu(link, id) {
      ...
      // Toggle การขยาย/ปิดเมนูย่อย
      if (this.expandedMenus.includes(link)) {
        // ถ้าเมนูนี้ถูกขยายอยู่แล้ว ให้ปิด
        this.expandedMenus = this.expandedMenus.filter(menu => menu !== link);
      } else {
        // ถ้าเมนูนี้ยังไม่ถูกขยาย ให้เปิด
        this.expandedMenus.push(link);
      }
      
      // บันทึกสถานะการขยายเมนู
      ...
      if (menuItem && menuItem.child_obj && menuItem.child_obj.length > 0) {
        ...
        if (firstActiveChild && this.expandedMenus.includes(link)) {
          // Auto-select the first active child เฉพาะเมื่อเมนูถูกขยาย
          ...
          // No active children หรือเมนูถูกปิด, just clear child selection
          this.activeIdMenuChild = '';
          localStorage.setItem("activepath", link);
          localStorage.removeItem("activepathHeader");
          this.$router.push(link);
      ...
      console.log("Expanded menus:", this.expandedMenus);
    },
