<template>
  <div id="sidebar" style="overflow:initial;" class="scroll">
    <v-navigation-drawer v-model="isSelectedChild" app class="elevation-3 pr-3" height="100%" width="300">
      ...
      <ul v-for="item in dataMenuBar" :key="item.name">
        ...
        <div v-else-if="item.id === 11"></div>
        <div v-else>
          <li class="item pointer" @click="other(item.path, item.id)" v-bind:class="{ active: item.path === activeIdOther }">
            <b>{{ item.name }}</b>
          </li>
        </div>
