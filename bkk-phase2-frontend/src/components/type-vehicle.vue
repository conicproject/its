<template>
  <v-card class="elevation-0" id="border" v-if="$vuetify.breakpoint.width > 900"  >
    <v-row>
      <v-col class="pb-0">
        <v-container class="pa-0">
          <v-list-item v-for="item in Vehtype_1" :key="'A'+item.id">
            <v-list-item-title class="pa-0">
              <v-row class="pb-1">
              
                <v-col cols="3" class="text-center pt-5 pb-0" ><img width="100%" heigh="100%" :src="require(`@/assets/img/vehicle/${item.image}`)"/></v-col>
              ...
              <v-col class="mt-0 mb-1">
                ...
                <v-col  class="text-right"
                  ><span class="textNum">{{ item.dataWithComma }}</span
                  ><span class="textUnit">&nbsp;&nbsp;คัน</span></v-col
                >
      ...
      <v-col class="pb-0">
        <v-container class="pa-0">
          <v-list-item v-for="item in Vehtype_2" :key="'b'+item.id">
            <v-list-item-title class="pa-0">
              <v-row class="pb-1">
  ...
  <v-card class="elevation-0" id="border" v-else >
    <v-container class="pa-0">
      <v-list-item v-for="item in Vehtype_1" :key="'c'+item.id" class="pt-3">
        <v-list-item-title class="pa-0">
          <v-row class="pa-0">
            <v-col cols="2"><img width="100%" height="100%" :src="require(`@/assets/img/vehicle/${item.image}`)"/></v-col>
            <v-col
              ><span class="textVeh">{{ item.name }}</span>
            </v-col>
            <v-col class="text-right"
      ...
      <v-list-item v-for="item in Vehtype_2" :key="'d'+item.id" class="pb-3">
        <v-list-item-title class="pa-0">
          <v-row class="pa-0">
            <v-col cols="2"><img width="100%" height="100%" :src="require(`@/assets/img/vehicle/${item.image}`)"/></v-col>
            <v-col
    ...
    vehicle: function(newVal, oldVal) {
     ...
     this.Vehtype_1 = newVal.volume
          ...
          .map((item,index) => ({
            ...
            percent: newVal.percent[index]['percent'].toFixed(0) 
          }));
          ...
          .map((item,index) => ({
            ...
            data: item.volume,
            dataWithComma: this.toCommas(item.volume),
            percent: newVal.percent[5+index]['percent'].toFixed(0) 
          }));
