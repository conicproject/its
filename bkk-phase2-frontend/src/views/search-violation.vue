  ...
  <v-app id="bg">
    ...
    <v-content class="pl-3" v-if="$vuetify.breakpoint.width > 1025">
      <v-container class="pl-1 pr-1">
        ...
        <!-- <v-layout><v-chip large class="ma-2" color="#02754B">
            <span class="pl-5 pr-5 headertext">ค้นหาข้อมูลยานพาหนะที่ฝ่าฝืนสัญญาณไฟจราจร</span>
        </v-chip></v-layout> -->
...
// import formsearch from "../components/form-search-violation.vue";
...
export default {
  ...
  methods: {
    ...
    confirm(carType, color, point, province, direction, date, license) {
      ...
      localStorage.setItem('vehicleViolationParam', JSON.stringify(param))
      ...
      this.axios.get(url, { headers: { session: session_data }, params: param })
        .then((response) => {
          if (response.status === 200) {
            ...
            localStorage.setItem('vehicleViolationFilter', JSON.stringify(filter))

            this.$router.push({ path: '/search-violation-table' });
      ...
      // this.$router.push({ path: '/search-violation-table', query: param });
