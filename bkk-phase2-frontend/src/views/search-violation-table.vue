    ...
    <v-main class="pl-3">
     ...
     <v-icon large color="red">mdi-chevron-left</v-icon>
        ...
        <v-chip  v-if="$vuetify.breakpoint.width > 750" large class="ma-2" color="#02754B">
          <span class="pl-5 pr-5 headertext">ค้นหายานพาหนะที่ฝ่าฝืนสัญญาณไฟจราจรทางข้ามชนิดปุ่มกด</span>
        </v-chip>
                <v-chip v-else large class="ma-2" color="#02754B">
         <span  class="pl-5 pr-5 headertext-mobile">ค้นหายานพาหนะที่ฝ่าฝืน<br/>สัญญาณไฟจราจรทางข้ามชนิดปุ่มกด</span>
        </v-chip>  
        
        <searchdetail :checkPage="checkPage" :header="headers" :dataTable="dataTable" :export="exportF" :dataFilter="dataFilter"
        ...
        <exportfile class="pt-4 pr-2"  :keypage="keypage" @download="generateReport" @downloadEx="genEX"
...
>
  Download CSV (you can customize this with html code!)
    ...
    <searchreportpdf :downloadpdf="downloadpdf" :downloadexcel="downloadexcel"
      :headers="headers" :dataTableRe="dataTable" :loading="onLoading"></searchreportpdf>
...
import searchdetail from "../components/search-violation-table.vue";
import exportfile from "../components/export-file.vue";
import headerbar from "../components/header-bar.vue";
import searchreportpdf from "../formatExport/reportViolationPDF.vue";
...
export default {
  ...
  methods: {
    ...
    gobackpage(){
      this.$router.push('/vehicle/search-violation');
    },
    setDateReport(param) {
    ...
    getData() {
      ...
      let url = baseUrl.ipServer + "/vehicle-violation-search";
      this.onLoading = true
      this.axios
        .get(url, {
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            this.dataTable = [];
            let blankImg = "data:image/png;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
            ...
            for (let i = 0; i < response.data.data.records.length; i++) {
              ...
              if(response.data.data.records[i]["image_url"] !== "None" && dataRes["image_url"] !== null){
              ...
              if(response.data.data.records[i]["plate_picture"] !== "None" && dataRes["plate_picture"] !== null){
              ...
              // data["img_base64"] = response.data.data.records[i]['base64_plate_pic'] ?'data:image/png;base64,'+response.data.data.records[i]['base64_plate_pic'] : ''
              ...
              if(isPublic === 0){
                data["img_base64"] = dataRes['base64_plate_pic'] ?'data:image/png;base64,'+dataRes['base64_plate_pic'] : ''
              } else {
                if(dataRes['base64_plate_pic']){
                  data["img_base64"] = dataRes['base64_plate_pic'] ?'data:image/png;base64,'+dataRes['base64_plate_pic'] : ''
                } else {
    ...
    setDataFilter() {
      const filter = localStorage.getItem('vehicleViolationFilter')
      const param = localStorage.getItem('vehicleViolationParam')
      ...
      this.dataFilter = {
        ...
        'point': filter_parse["checkpoint"]["short_name"],
        'direction': direction || "-",
      ...
      this.axios
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            this.onLoading = false
            localStorage.setItem('vehicleViolationParam', JSON.stringify(params))
          }
        })
        .catch((error) => {
          this.onLoading = false;
